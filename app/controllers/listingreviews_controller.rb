class ListingreviewsController < ApplicationController
  def index
    ...
    @listingreviews = @q.result(:distinct => true).includes(:listing, :user).page(params[:page]).per(10)
  ...
  def create
    ...
    @listingreview.user_id = params[:user_id]
    ...
    @listingreview.content = params[:content]
